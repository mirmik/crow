...
public:
	pybind_requestor(crow::hostaddr addr, std::string theme, std::string reply_theme)
		: crow::requestor_node()
	{
		this->addr = addr;
		this-> theme = theme;
	...
	)
	{
		...
		requestor_node::async_request(igris::buffer{ 
			info.data(), 
			info.size() });
	}
	...
	{
		std::string info = data;
		return requestor_node::request(igris::buffer{ 
			info.data(), 
			info.size() });
	}
};
...
void register_requestor_class(py::module & m)
{
	...
	.def(py::init<crow::hostaddr, std::string, std::string>(), py::arg("addr"), py::arg("theme"), py::arg("reply_theme"))
