    ...
    class service_node : public crow::abstract_subscriber_node
    {
      ...
      public:
        ...
        service_node(const crow::hostaddr_view &addr,
                     const igris::buffer &theme, const delegate &dlg)
        ...
        void set_handle(const delegate &dlg) { this->dlg = dlg; }

        void init(const delegate &dlg) { this->dlg = dlg; }

        void init(const crow::hostaddr_view &addr, const igris::buffer &theme)
        {
            abstract_subscriber_node::init_subscribe(addr, theme, 2, 50, 2, 50);
        }

        void reply(const char *data, size_t size);
        void reply(igris::buffer buf) { reply(buf.data(), buf.size()); }
