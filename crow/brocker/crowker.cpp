...
void crow::crowker::crow_subscribe(const crow::hostaddr & addr,
                             ...
                             uint8_t qos, uint16_t ackquant)
{
	...
	if (!thm->has_subscriber(sub))
	{
		...
		if (brocker_info)
			nos::fprintln("new subscribe(crow): a:{} q:{} c:{} t:{}", igris::dstring(addr.data(), addr.size()), qos, ackquant, theme);
	} else 
	{
		...
		if (opts->qos != qos  || opts->ackquant != ackquant) 
		{
			...
			nos::fprintln("change subscribe(crow): a:{} q:{} c:{} t:{}", igris::dstring(addr.data(), addr.size()), qos, ackquant, theme);		
		}
	}
}

void crow::crowker::tcp_subscribe(const std::string& theme, nos::inet::tcp_socket * sock)
{
