 ...
 * @brief Tests for crow packet pool allocator
 *
 * Note: These tests require linking with allocation_pool.cpp instead of
 * allocation_malloc.cpp. The test verifies pool-based packet allocation.
 */
...
// Test uses pool allocator, so we need to initialize the pool
// before running tests. This is done in a test case.
