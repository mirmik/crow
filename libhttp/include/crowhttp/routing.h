...
#include "crowhttp/exceptions.h"
#include "crowhttp/websocket.h"
#include "crowhttp/mustache.h"
#include "crowhttp/middleware.h"

namespace crowhttp // NOTE: Already documented in "crowhttp/app.h"
{
    ...
    namespace detail
    {
        ...
        struct middleware_indices
        {
            ...
            void push()
            {
                ...
                static_assert(std::is_base_of<crowhttp::ILocalMiddleware, MW>::value, "Middleware must extend ILocalMiddleware");
    ...
    namespace detail
    {
        namespace routing_handler_call_helper
        {
            ...
            struct Wrapped
            {
                ...
                {
                    using type = std::function<void(const crowhttp::request&, crowhttp::response&, Args...)>;
    ...
    public:
          ...
          operator()(Func&& f)
        {
            static_assert(!std::is_same<void, decltype(f())>::value,
                          "Handler function cannot have void return type; valid return types: string, int, crowhttp::response, crowhttp::returnable");
        ...
        typename std::enable_if<
          !black_magic::CallHelper<Func, black_magic::S<>>::value &&
            black_magic::CallHelper<Func, black_magic::S<crowhttp::request>>::value,
          void>::type
          operator()(Func&& f)
        {
            static_assert(!std::is_same<void, decltype(f(std::declval<crowhttp::request>()))>::value,
                          "Handler function cannot have void return type; valid return types: string, int, crowhttp::response, crowhttp::returnable");
        ...
        typename std::enable_if<
          !black_magic::CallHelper<Func, black_magic::S<>>::value &&
            !black_magic::CallHelper<Func, black_magic::S<crowhttp::request>>::value &&
            black_magic::CallHelper<Func, black_magic::S<crowhttp::response&>>::value,
          void>::type
          operator()(Func&& f)
        {
            static_assert(std::is_same<void, decltype(f(std::declval<crowhttp::response&>()))>::value,
        ...
        typename std::enable_if<
          ...
          operator()(Func&& f)
        {
            static_assert(std::is_same<void, decltype(f(std::declval<crowhttp::request>(), std::declval<crowhttp::response&>()))>::value,
                          "Handler function with response argument should have void return type");
    ...
    private:
        std::function<void(const crowhttp::request&, crowhttp::response&)> handler_;
    };
    ...
    public:
        ...
        void handle_upgrade(const request& req, response&, SocketAdaptor&& adaptor) override
        {
            ...
            crowhttp::websocket::Connection<SocketAdaptor, App>::create(req, std::move(adaptor), app_, max_payload_, subprotocols_, open_handler_, message_handler_, close_handler_, error_handler_, accept_handler_, mirror_protocols_);
        }

        void handle_upgrade(const request& req, response&, UnixSocketAdaptor&& adaptor) override
        {
            max_payload_ = max_payload_override_ ? max_payload_ : app_->websocket_max_payload();
            crowhttp::websocket::Connection<UnixSocketAdaptor, App>::create(req, std::move(adaptor), app_, max_payload_, subprotocols_, open_handler_, message_handler_, close_handler_, error_handler_, accept_handler_, mirror_protocols_);
        }

#ifdef CROW_ENABLE_SSL
        void handle_upgrade(const request& req, response&, SSLAdaptor&& adaptor) override
        {
            crowhttp::websocket::Connection<SSLAdaptor, App>::create(req, std::move(adaptor), app_, max_payload_, subprotocols_, open_handler_, message_handler_, close_handler_, error_handler_, accept_handler_, mirror_protocols_);
        }
#endif
        ...
        self_t& onaccept(std::function<void(const crowhttp::request&, std::optional<crowhttp::response>&, void**)>&& callback)
        {
            accept_handler_ = std::move(callback);
            return *this;
        }

        self_t& onaccept(std::function<bool(const crowhttp::request&, void**)>&& callback)
        {
            onaccept([callback](const crowhttp::request& req, std::optional<crowhttp::response>& res, void** p) {
                if (!callback(req, p))
                {
                    res = crowhttp::response(400);
                }
            });
            return *this;
        }
    ...
    protected:
        App* app_;
        std::function<void(crowhttp::websocket::connection&)> open_handler_;
        std::function<void(crowhttp::websocket::connection&, const std::string&, bool)> message_handler_;
        std::function<void(crowhttp::websocket::connection&, const std::string&, uint16_t)> close_handler_;
        std::function<void(crowhttp::websocket::connection&, const std::string&)> error_handler_;
        std::function<void(const crowhttp::request&, std::optional<crowhttp::response>&, void**)> accept_handler_;
    ...
    private:
        std::function<void(crowhttp::request&, crowhttp::response&, Args...)> handler_;
    };
...
*/
    ...
    public:
        ...
        std::unique_ptr<routing_handle_result> handle_initial(request& req, response& res)
        {
            ...
            else if (req.method == HTTPMethod::Options)
            {
                ...
                if (req.url == "/*")
                {
                    ...
                    res = response(crowhttp::status::OK);
#else
                    res = response(crowhttp::status::NO_CONTENT);
#endif
        ...
        typename std::enable_if<std::tuple_size<typename App::mw_container_t>::value != 0, void>::type
          handle_rule(BaseRule& rule, crowhttp::request& req, crowhttp::response& res, const crowhttp::routing_params& rp)
        {
        ...
        std::function<void(crowhttp::response&)>& exception_handler()
        {
    ...
    private:
        ...
        std::vector<std::unique_ptr<BaseRule>> all_rules_;
        std::vector<Blueprint*> blueprints_;
        std::function<void(crowhttp::response&)> exception_handler_ = &default_exception_handler;
    };
} // namespace crowhttp
