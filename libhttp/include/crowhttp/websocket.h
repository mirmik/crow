...
namespace crowhttp // NOTE: Already documented in "crowhttp/app.h"
{
    ...
    namespace websocket
    {
        ...
        public:
            ...
            static void create(const crowhttp::request& req, Adaptor adaptor, Handler* handler,
                               uint64_t max_payload, const std::vector<std::string>& subprotocols,
                               std::function<void(crowhttp::websocket::connection&)> open_handler,
                               std::function<void(crowhttp::websocket::connection&, const std::string&, bool)> message_handler,
                               std::function<void(crowhttp::websocket::connection&, const std::string&, uint16_t)> close_handler,
                               std::function<void(crowhttp::websocket::connection&, const std::string&)> error_handler,
                               std::function<void(const crowhttp::request&, std::optional<crowhttp::response>&, void**)> accept_handler,
                               bool mirror_protocols)
            {
                ...
                {
                    void* ud = nullptr;
                    std::optional<crowhttp::response> res;
                ...
                conn->start(crowhttp::utility::base64encode((unsigned char*)digest, 20));
            }
        ...
        private:
            ...
            std::function<void(crowhttp::websocket::connection&, const std::string&, uint16_t status_code)> close_handler_;
