/**
 * \file crow/app.h
 * \brief This file includes the definition of the crowhttp::Crow class,
 * the crowhttp::App and crowhttp::SimpleApp aliases, and some macros.
 *
 ...
 *
 * It use crowhttp::Crow::route_dynamic or crowhttp::Crow::route to define
 * a rule for your application. It's usage is like this:
 *
 * ```cpp
 * auto app = crowhttp::SimpleApp(); // or crowhttp::App()
 * CROW_ROUTE(app, "/")
 * ([](){
 ...
 */
#define CROW_ROUTE(app, url) app.template route<crowhttp::black_magic::get_parameter_tag(url)>(url)

/**
 * \def CROW_BP_ROUTE(blueprint, url)
 * \brief Creates a route for a blueprint using a rule.
 *
 * It may use crowhttp::Blueprint::new_rule_dynamic or
 * crowhttp::Blueprint::new_rule_tagged to define a new rule for
 * an given blueprint. It's usage is similar
 * to CROW_ROUTE macro:
 *
 * ```cpp
 * crowhttp::Blueprint my_bp();
 * CROW_BP_ROUTE(my_bp, "/")
 * ([](){
 *     return "<h1>Hello, world!</h1>";
 * });
 ...
 */
#define CROW_BP_ROUTE(blueprint, url) blueprint.new_rule_tagged<crowhttp::black_magic::get_parameter_tag(url)>(url)
 ...
 *                do_something();
 *            })
 *     .onclose([&](crowhttp::websocket::connection& conn, const std::string& reason, uint16_t){
 *                 do_something();
 *             })
 *     .onmessage([&](crowhttp::websocket::connection&, const std::string& data, bool is_binary){
 *                   if (is_binary)
 *                       do_something(data);
 *                   else
 ...
 */
#define CROW_WEBSOCKET_ROUTE(app, url) app.route<crowhttp::black_magic::get_parameter_tag(url)>(url).websocket<std::remove_reference<decltype(app)>::type>(&app)
 ...
 * can be used in both crowhttp::SimpleApp (and crowhttp::App) instances and
 * crowhttp::Blueprint. Its usage syntax is like this:
 *
 * ```cpp
 * auto app = crowhttp::SimpleApp(); // or crowhttp::App()
...
namespace crowhttp
{
     ...
     *
     * Use crowhttp::SimpleApp or crowhttp::App<Middleware1, Middleware2, etc...> instead of
    ...
    public:
        ...
        ///
        /// Possible values are:
        /// - crowhttp::LogLevel::Debug       (0)
        /// - crowhttp::LogLevel::Info        (1)
        /// - crowhttp::LogLevel::Warning     (2)
        /// - crowhttp::LogLevel::Error       (3)
        /// - crowhttp::LogLevel::Critical    (4)
        self_t& loglevel(LogLevel level)
        {
            crowhttp::logger::setLogLevel(level);
            return *this;
        }
        ...
        ///
        /// The function must have the following signature: void(crowhttp::response&).
        ...
        void add_blueprint()
        {
            ...
            for (Blueprint* bp : router_.blueprints())
            {
                ...
                auto static_dir_ = crowhttp::utility::normalize_path(bp->static_dir());

                bp->new_rule_tagged<crowhttp::black_magic::get_parameter_tag(CROW_STATIC_ENDPOINT)>(CROW_STATIC_ENDPOINT)([static_dir_](crowhttp::response& res, std::string file_path_partial) {
        ...
        void add_static_dir()
        {
            ...
            auto static_dir_ = crowhttp::utility::normalize_path(CROW_STATIC_DIRECTORY);

            route<crowhttp::black_magic::get_parameter_tag(CROW_STATIC_ENDPOINT)>(CROW_STATIC_ENDPOINT)([static_dir_](crowhttp::response& res, std::string file_path_partial) {
                utility::sanitize_filename(file_path_partial);
                res.set_static_file_info_unsafe(static_dir_ + file_path_partial);
                res.end();
            });
